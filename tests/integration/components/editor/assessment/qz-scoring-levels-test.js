...
test('Scoring Level Layout', function(assert) {
  ...
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .scale').length,'Missing scale');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .scale span.best').length,'Missing best label');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .scale .line').length,'Missing line');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .scale .arrow-down').length,'Missing arrow');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .scale .worst').length,'Missing worst label');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .scale .worst').length,'Missing worst label');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .levels .level-list').length,'Missing level list');
  assert.equal($component.find('.editor.assessment.qz-scoring-levels .level .levels .level-list .gru-input').length,4,'Should have 4 levels');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .level .btn-new-level').length,'Missing add new level button');
  assert.ok($component.find('.editor.assessment.qz-scoring-levels .points').length,'Missing points section');
