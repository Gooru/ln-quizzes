...
test('setAnswers', function(assert) {
  let component = this.subject();
  let userAnswer =[{value:1}];

  component.set('userAnswer',userAnswer);
  component.setAnswers();
  assert.deepEqual(component.get('selectedAnswers'),[{value:1}], 'Incorrect answers');
});

test('notify on load and change', function(assert) {
  assert.expect(2);
  let component = this.subject({
    notifyAnswerChanged: answerId => assert.deepEqual(answerId, [{value:1}, {value:3}], 'Answer should match in answer changed'),
    notifyAnswerLoaded: answerId => assert.deepEqual(answerId, [{value:1}, {value:3}], 'Answer should match in answer loaded'),
    notifyAnswerCompleted: () => assert.ok(false, 'Answer completed should not be called'),
    notifyAnswerCleared: () => assert.ok(false, 'Answer cleared should be called')
  });
...
test('notify clear and change', function(assert) {
  assert.expect(2);
  let component = this.subject({
    notifyAnswerChanged:answerId => assert.deepEqual(answerId, [], 'Answer should match in answer changed'),
    notifyAnswerLoaded: () =>  assert.ok(false, 'Answer cleared should be called'),
    notifyAnswerCompleted: () => assert.ok(false, 'Answer completed should not be called'),
    notifyAnswerCleared: answer => assert.deepEqual(answer, [], 'Answer should be cleared')
  });
