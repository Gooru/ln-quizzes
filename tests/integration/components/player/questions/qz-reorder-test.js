...
test('Notifications work after reordering questions', function (assert) {
  ...
  this.on('changeAnswer', function (question, answer) {
    //called 3 times
    assert.deepEqual(answer, answers, 'Answer changed, but the answers are not in the correct order');
  });

  this.on('completeAnswer', function (question, answer) {
    //called 2 times
    assert.deepEqual(answer, answers, 'Answer completed, but the answers are not in the correct order');
  });
...
test('Reorder question layout - read only', function (assert) {
  ...
  this.render(hbs`{{player/questions/qz-reorder question=question readOnly=readOnly}}`);
