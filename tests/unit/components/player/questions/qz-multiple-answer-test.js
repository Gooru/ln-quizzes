...
test('answers', function(assert) {
  ...
  assert.deepEqual(component.get('answers'), expectedAnswers, 'Answers should match');

  component.set('userSelection', Ember.A([
...
test('userSelection with no user answer', function(assert) {
  ...
  assert.deepEqual(component.get('userSelection'), [], 'User selection should match');
});
...
test('isAnswerCompleted not complete', function(assert) {
  ...
  assert.notOk(component.get('isAnswerCompleted').call(component), 'Answer should not be completed');
});
...
test('isAnswerCompleted complete', function(assert) {
  ...
  assert.ok(component.get('isAnswerCompleted').call(component), 'Answer should be completed');
});

test('choiceToUserSelectionItem', function(assert) {
  assert.expect(1);
  let component = this.subject();
  let expectedUserSelection = { value: 'answer', selection: true };
  let response = component.get('choiceToUserSelectionItem')('yes|answer');
  assert.deepEqual(response, expectedUserSelection, 'User selection should match');
});

test('userSelectionItemToChoice', function(assert) {
  assert.expect(1);
  let component = this.subject();
  let selection = { value: 'answer', selection: false };
  let response = component.get('userSelectionItemToChoice')(selection);
  let expectedValue = 'no|answer';
...
test('notify completed on load', function(assert) {
  ...
  let component = this.subject({
    ...
    question: ResourceModel.create({
      answers:  Ember.A([
        AnswerModel.create({ value: 'answer', text: 'text1' })
      ])
    })
  });
  component.set('userSelection', [{
