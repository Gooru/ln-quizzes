...
moduleForComponent('reports/assessment/qz-summary', 'Integration | Component | reports/assessment/qz summary', {
  integration: true,
...
test('it renders for assessment', function (assert) {
  ...
  let $component = this.$('.reports.assessment.qz-summary');  //component dom element
  assert.ok($component.length, 'Component does not have the component classes');

  let $gradeContainer = $component.find('.summary-container .grade[style~=\'background-color:\']');
  assert.ok($gradeContainer.length, 'Grade container is missing');

  let $percentage = $gradeContainer.find('.percentage');
  ...
  let $attempts = $gradeContainer.find('.attempts');
  ...
  let $fractional = $attempts.find('.fractional');
  ...
  let $overviewContainer = $component.find('.summary-container .overview');
  ...
  // Links to questions
  let $questionLinks = $overviewContainer.find('.gru-bubbles');
...
test('it renders for collection', function (assert) {
  ...
  this.render(hbs`{{reports/assessment/qz-summary contextResult=contextResult}}`);

  let $component = this.$('.reports.assessment.qz-summary');  //component dom element
  assert.ok($component.length, 'Component does not have the component classes');

  let $gradeContainer = $component.find('.summary-container .grade');
  assert.ok(!$gradeContainer.length, 'Grade container should be visible');

  let $thumbnailContainer = $component.find('.summary-container .thumbnail');
