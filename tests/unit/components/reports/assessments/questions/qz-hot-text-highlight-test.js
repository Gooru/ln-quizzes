...
moduleForComponent('reports/assessment/questions/qz-hot-text-highlight', 'Unit | Component | reports/assessment/questions/qz hot text highlight', {
...
test('items when show correct', function(assert) {
  let mapItems = ({ index, text, selected, correct }) => ({ index, text, selected, correct });
  ...
  assert.deepEqual(component.get('items').map(mapItems), expectedItems, 'Incorrect answers');
});
...
test('items show user answers', function(assert) {
  ...
  component.set('question', question);
  component.set('userAnswer', [{ value: 'Sentence 2.,12' }, { value: 'Sentence 3.,24' }]);
  assert.deepEqual(component.get('items').map(mapItems), expectedItems, 'Incorrect answers');
});
