...
moduleForComponent('reports/assessment/questions/qz-open-ended', 'Unit | Component | reports/assessment/questions/qz open ended', {
...
test('answer when show correct', function(assert) {
  ...
  let expectedAnswers = 'answer';
  component.set('showCorrect', true);
  component.set('question', question);
  assert.deepEqual(component.get('answer'), expectedAnswers, 'Incorrect answers');
});
...
test('answers show user answers', function(assert) {
  ...
  let expectedAnswers = 'user-answer';
