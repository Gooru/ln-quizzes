...
test('Layout', function(assert) {
  ...
  assert.equal($submissionFormat.find('.panel.submission-format-textbox .panel-footer').text().trim(),this.get('i18n').t('qz-submission-format.textbox').string, 'Incorrect textbox label');
  assert.equal($submissionFormat.find('.panel.submission-format-upload .panel-footer').text().trim(),this.get('i18n').t('qz-submission-format.upload').string, 'Incorrect upload label');
});
