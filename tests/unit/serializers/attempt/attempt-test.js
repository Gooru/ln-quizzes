...
test('normalizeReportEvents', function(assert) {
  ...
  assert.ok(response[0].get('isAttemptFinished'), 'Wrong first is attempt finished');
  ...
  assert.notOk(response[1].get('isAttemptFinished'), 'Wrong second is attempt finished');
});
...
test('normalizeReportEvent', function(assert) {
  ...
  assert.equal(response.get('collectionId'), 'collection-id', 'Wrong collection id');
  assert.equal(response.get('contextId'), 'context-id', 'Wrong context id');
  assert.equal(response.get('currentResourceId'), 'current-resource-id', 'Wrong current resource');
